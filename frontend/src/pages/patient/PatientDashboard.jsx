    ...
    <div>
      ...
      {appointments.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <Link to="/patient/create-appointment" className="text-primary hover:text-primaryDark font-medium mt-2 inline-block">
            Solicita tu primer turno
          </Link>
        </div>
      ) : (
            ...
            <div key={appointment.id} className="bg-white rounded-xl shadow-sm p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-800">{appointment.professionalName}</h3>
              ...
              {appointment.notes && (
                <div className="mt-4 pt-4 border-t">
                  <p className="text-sm text-gray-600"><strong>Notas:</strong> {appointment.notes}</p>
                </div>
              )}
            </div>
          ))}
